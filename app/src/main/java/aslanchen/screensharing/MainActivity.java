...
public class MainActivity extends Activity implements OnItemClickListener,
    ...
    private void InitData() {
        ...
        outRunnable = new Runnable() {
            ...
            @Override
            public void run() {
                Toast.makeText(MainActivity.this, "连接超时,请重启连接设备！",
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position,
        ...
        if (position == connectIndex) {
            // 断开
            try {
                Method disconnectWifiDisplay = mDisplayManager.getClass()
            ...
            // 连接
            try {
                Method connectWifiDisplay = mDisplayManager.getClass()
                        .getMethod("connectWifiDisplay", String.class);
                connectWifiDisplay.invoke(mDisplayManager,
                        listAvilable.get(position).getDeviceAddress());
    ...
    private void startScan() {
        try {
            // 反射出扫描方法
            Method scanWifiDisplays = mDisplayManager.getClass().getMethod(
                    "scanWifiDisplays");
            // 反射出获取扫描结果方法
            getWifiDisplayStatus = mDisplayManager.getClass().getMethod(
    ...
    private void getAvivableDrive() {
        ...
        try {
            ...
            // 获取支持的设备值
            Field mAvailableDisplays = mWifiDisplayStatus.getClass()
                    .getDeclaredField("mAvailableDisplays");
            mAvailableDisplays.setAccessible(true);// 设置安全检查，访问私有成员变量必须
            Object mAvailablefildValue = mAvailableDisplays
                    .get(mWifiDisplayStatus);
            ...
            // 获取记住的设备
            Field mRememberedDisplays = mWifiDisplayStatus.getClass()
                    .getDeclaredField("mRememberedDisplays");
            mRememberedDisplays.setAccessible(true);// 设置安全检查，访问私有成员变量必须
            Object mRememberedfildValue = mRememberedDisplays
                    .get(mWifiDisplayStatus);
    ...
    private void getActiveDisplayValue(Object[] osAva) {
        try {
            // 获取当前连接屏幕
            Field mActiveDisplay = mWifiDisplayStatus.getClass()
                    .getDeclaredField("mActiveDisplay");
    ...
    private void getActiveDisplayState() {
        // 获取当前连接状态
        try {
            Field mActiveDisplayState = mWifiDisplayStatus.getClass()
                    .getDeclaredField("mActiveDisplayState");
            mActiveDisplayState.setAccessible(true);// 设置安全检查，访问私有成员变量必须
            Object mActiveDisplayStateValue = mActiveDisplayState
                    .get(mWifiDisplayStatus);
    ...
    private WifiDisplayPro getValue(Object clas) throws NoSuchFieldException,
            IllegalArgumentException, IllegalAccessException {
        WifiDisplayPro wdp = new WifiDisplayPro();
        String[] names = new String[]{"mDeviceAddress", "mDeviceAlias",
                "mDeviceName", "mDeviceType", "mGroupCapability"};
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            } else if ("android.hardware.display.action.WIFI_DISPLAY_STATUS_CHANGED"
                    .equals(action) == true) {
                // 连接上，断开会触发这个
